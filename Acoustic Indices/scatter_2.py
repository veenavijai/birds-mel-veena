...
signal, rate = librosa.load(file_path, sr=sample_rate)

fs = 16000
win_len_s = 20 / 1000       # in ms
win_len = np.ceil(win_len_s * fs).astype(np.int)                #this is window length in samples
nfft = 2 ** np.ceil(np.log2(win_len)).astype(np.int)            # nextpow2 of window length is set as size of FFT
    
S = stft(signal, fs, window='hann', nperseg=win_len, noverlap=None, nfft=nfft, detrend=False, return_onesided=True, boundary='zeros', padded=True, axis=-1)
print(S)'''
